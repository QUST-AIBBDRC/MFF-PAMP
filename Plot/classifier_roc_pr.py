...
plt.subplot(121)
A = open('label_RandomForest.csv')
B = open('probability_RandomForest.csv')
label = pd.read_csv(A)
...
roc_auc = auc(fpr, tpr)
plt.plot(fpr, tpr, color='blue',lw=lw,label='Random Forest(AUC=0.9978)')

A = open('label_NaiveBayes.csv')
B = open('probability_NaiveBayes.csv')
label = pd.read_csv(A)
score_= pd.read_csv(B)
label = np.array(label)
score_= np.array(score_)
...
fpr, tpr, _ = roc_curve(ytest_GTB[:,0], ytest_GTB[:,2])
roc_auc = auc(fpr, tpr)
plt.plot(fpr, tpr, color='brown',lw=lw,label='Naïve Bayes(AUC=0.8747)')

A = open('label_DecisionTree.csv')
B = open('probability_DecisionTree.csv')
label = pd.read_csv(A)
score_= pd.read_csv(B)
label = np.array(label)
score_= np.array(score_)
...
fpr, tpr, _ = roc_curve(ytest_GTB[:,0], ytest_GTB[:,2])
roc_auc = auc(fpr, tpr)
plt.plot(fpr, tpr, color='red',lw=lw,label='Decision Tree(AUC=0.9976)')

A = open('label_LogisticRegression.csv')
B = open('probability_LogisticRegression.csv')
label = pd.read_csv(A)
score_= pd.read_csv(B)
label = np.array(label)
score_= np.array(score_)
...
fpr, tpr, _ = roc_curve(ytest_GTB[:,0], ytest_GTB[:,2])
roc_auc = auc(fpr, tpr)
plt.plot(fpr, tpr, color='green',lw=lw,label='Logistic Regression(AUC=0.9765)')
...
plt.plot(fpr, tpr, color='blue',lw=lw,label='Random Forest(AUPR=0.9974)')
...
plt.plot(fpr, tpr, color='brown',lw=lw,label='Naïve Bayes(AUPR=0.7689)')
...
plt.plot(fpr, tpr, color='red',lw=lw,label='Decision Tree(AUPR=0.9684)')
...
plt.plot(fpr, tpr, color='green',lw=lw,label='Logistic Regression(AUPR=0.9608)')
ax = plt.gca()
ax.spines['left'].set_linewidth(0.3)
ax.spines['right'].set_linewidth(0.3)
ax.spines['bottom'].set_linewidth(0.3)
...
fig_size.set_size_inches(7,3.15)
plt.show
plt.savefig(r'classfier_auc_pr.svg',format='svg',dpi=2000)